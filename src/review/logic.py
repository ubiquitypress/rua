...
def get_editors(review_assignment):
    ...
    if review_assignment.book.series:
        ...
        if series_editor:
            ...
            press_editor_list = [{'editor': editor, 'isSeriesEditor': False} for
    ...
    if book_editors:
        book_editor_list = [{'editor': editor, 'isSeriesEditor': False} for
                            editor in book_editors if
                            not editor in press_editors]
    else:
        book_editor_list = []
...
def notify_editors(book, message, editors, creator, workflow):
    for editor_details in editors:
        notification = core_models.Task(book=book,
                                        assignee=editor_details['editor'],
                                        creator=creator, text=message,
                                        workflow=workflow)
        notification.save()
