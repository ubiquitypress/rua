	...
	def __init__(self, *args, **kwargs):
		...
		for relation in relations:
			...
			if relation.element.field_type == 'text':
				...
				self.fields[relation.element.name] = forms.ChoiceField(widget=forms.Select(attrs={'div_class':relation.element_class}), choices=choices, required=relation.element.required)
			elif relation.element.field_type == 'email':
				self.fields[relation.element.name] = forms.EmailField(widget=forms.TextInput(attrs={'div_class':relation.element_class}), required=relation.element.required)
