...
def delete_incomplete_proposal(request, proposal_id):
    incomplete_proposal = get_object_or_404(
        submission_models.IncompleteProposal, pk=proposal_id)
...
def incomplete_proposal(request, proposal_id):
    ...
    default_fields_validated = manager_forms.DefaultForm(
        initial={'title': incomplete_proposal.title,
    ...
    if request.method == 'POST' and 'book_submit' in request.POST:
        ...
        if proposal_form.is_valid() and default_fields.is_valid():
            ...
            message = "A new  Proposal '%s' with id %s has been submitted by %s ." % (
            proposal.title,
            proposal.pk,
            ...
            email_text = core_models.Setting.objects.get(group__name='email',
                                                         name='proposal_submission_ack').value
            core_logic.send_proposal_submission_ack(proposal,
                                                    email_text=email_text,
                                                    owner=request.user)

            log.add_proposal_log_entry(proposal=proposal, user=request.user,
                                       kind='proposal',
            ...
            proposal_form = manager_forms.GeneratedForm(request.POST,
                                                        request.FILES,
                                                        form=core_models.ProposalForm.objects.get(
                                                            pk=proposal_form_id))
            default_fields = manager_forms.DefaultForm(request.POST)
        ...
        proposal_form = manager_forms.GeneratedNotRequiredForm(request.POST,
                                                               request.FILES,
                                                               form=core_models.ProposalForm.objects.get(
                                                                   pk=proposal_form_id))
        ...
        if proposal_form.is_valid() and default_fields.is_valid():
            defaults = {field.name: field.value() for field in default_fields}
            incomplete_proposal.form = core_models.ProposalForm.objects.get(
                pk=proposal_form_id)
            ...
            proposal_data_processing(request, incomplete_proposal,
                                     proposal_form_id)
            messages.add_message(request, messages.SUCCESS,
                                 'Proposal %s saved' % incomplete_proposal.id)
            return redirect(reverse('user_dashboard', kwargs={}))
...
def start_proposal(request):
    submit_proposals = core_models.Setting.objects.get(group__name='general',
                                                       name='submit_proposals').value
    ...
    for field in file_fields:
        if field.element.name in request.FILES:
            ...
            save_dict[field.element.name] = [
                handle_proposal_file_form(request.FILES[field.element.name],
                                          proposal, 'other', request.user,
                                          "Attachment: Uploaded by %s" % (
                                          request.user.username))]
...
def proposal_revisions(request, proposal_id):
    ...
    email_text = get_email_content(
        ...
        context={'sender': request.user, 'receiver': proposal.owner,
                 'proposal': proposal,
                 'press_name': core_models.Setting.objects.get(
                     group__name='general', name='press_name').value}
    )
    ...
    default_fields = manager_forms.DefaultForm(
        initial={'title': proposal.title, 'author': proposal.author,
    ...
    if request.POST:
        ...
        if proposal_form.is_valid() and default_fields.is_valid():
            ...
            proposal = submission_models.Proposal.objects.get(
                form=core_models.ProposalForm.objects.get(pk=proposal.form.id),
                owner=request.user, pk=proposal_id)
            ...
            if proposal_type:
                proposal.book_type = proposal_type
            proposal.save()
            notification = core_models.Task(assignee=proposal.requestor,
                                            creator=request.user,
            ...
            log.add_proposal_log_entry(proposal=proposal, user=request.user,
                                       kind='proposal',
                                       message='Revisions for Proposal "%s %s" with id %s have been submitted.' % (
                                       proposal.title, proposal.subtitle,
                                       proposal.pk),
                                       short_name='Proposal Revisions Submitted')
            core_logic.send_proposal_update(proposal,
...
def proposal_add_note(request, proposal_id):
    ...
    if not request.user.profile.is_editor():
        return redirect(reverse('proposal_view_submitted',
...
def proposal_update_note(request, proposal_id, note_id):
    ...
    note = get_object_or_404(submission_models.ProposalNote, proposal=proposal,
                             pk=note_id)
...
def proposal_history_view(request, proposal_id, history_id):
    ...
    proposal_form_id = core_models.Setting.objects.get(
        name='proposal_form').value
    proposal = get_object_or_404(submission_models.HistoryProposal,
                                 proposal=parent_proposal, pk=history_id)
...
def handle_file(file, book, kind, user):
    if file:
        ...
        original_filename = smart_text(file._get_name()).replace(',',
                                                                 '_').replace(
            ';', '_')
