...
class SubmitBookStageOne(forms.ModelForm):
    book_type = forms.CharField(required=True, widget=forms.Select(
        choices=core_models.book_type_choices()))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not review_type_required == 'on':
            self.fields['review_type'] = forms.CharField(required=False,
                                                         widget=forms.Select(
                                                             choices=core_models.book_review_type_choices()))
        else:
            self.fields['review_type'] = forms.CharField(required=True,
                                                         widget=forms.Select(
                                                             choices=core_models.book_review_type_choices()))
...
class SubmitBook(forms.ModelForm):
    class Meta:
        model = core_models.Book
        fields = ('title', 'subtitle', 'description', 'subject', 'license',
                  'cover_letter', 'series')
