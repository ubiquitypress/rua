...
def is_press_editor(function):
    def wrap(request, *args, **kwargs):
        ...
        if 'press-editor' in user_roles:
            ...
            messages.add_message(request, messages.ERROR,
                                 'You need to have Press Editor level permission to view this page.')
            raise exceptions.PermissionDenied
...
def is_production_editor(function):
    def wrap(request, *args, **kwargs):
        ...
        if 'production-editor' in user_roles:
            ...
            messages.add_message(request, messages.ERROR,
                                 'You need to have Production Editor level permission to view this page.')
            raise exceptions.PermissionDenied
...
def is_editor(function):
    def wrap(request, *args, **kwargs):
        ...
        if 'press-editor' in user_roles or 'series-editor' in user_roles or 'book-editor' in user_roles or 'production-editor' in user_roles:
            ...
            messages.add_message(request, messages.ERROR,
                                 'You need to have Press Editor, Book Editor or Series Editor level permission to view this page.')
            raise exceptions.PermissionDenied
...
def is_reviewer(function):
    def wrap(request, *args, **kwargs):
        ...
        if one_click_no_login:
            if one_click_no_login[0].value == 'on':
                ...
                if 'access_key' in full_url:
                    ...
                    proposal_review_assignments = submission_models.ProposalReview.objects.filter(
                        access_key=access_key)
        ...
        if 'press-editor' in user_roles:
            ...
            book = get_object_or_404(models.Book, pk=submission_id)
            if request.user in [reviewer.user for reviewer in
                                book.reviewassignment_set.all()] or book.owner == request.user:
                return function(request, *args, **kwargs)
            else:
                messages.add_message(request, messages.ERROR,
                                     'You need to have Press Editor, Book Editor or Series Editor level permission to view this page.')
...
def is_indexer(function):
    def wrap(request, *args, **kwargs):
        ...
        if 'press-editor' in user_roles:
            ...
            book = get_object_or_404(models.Book, pk=submission_id)
            if request.user in [reviewer.indexer for reviewer in
                                book.indexassignment_set.all()] or book.owner == request.user:
                return function(request, *args, **kwargs)
            else:
                messages.add_message(request, messages.ERROR,
                                     'You need to have Press Editor, Book Editor or Series Editor level permission to view this page.')
...
def is_copyeditor(function):
    def wrap(request, *args, **kwargs):
        ...
        if 'press-editor' in user_roles:
            ...
            book = get_object_or_404(models.Book, pk=submission_id)
            if request.user in [reviewer.copyeditor for reviewer in
                                book.copyeditassignment_set.all()] or book.owner == request.user:
                return function(request, *args, **kwargs)
            else:
                messages.add_message(request, messages.ERROR,
                                     'You need to have Press Editor, Book Editor or Series Editor level permission to view this page.')
...
def is_typesetter(function):
    def wrap(request, *args, **kwargs):
        ...
        if 'press-editor' in user_roles:
            ...
            book = get_object_or_404(models.Book, pk=submission_id)
            if request.user in [reviewer.typesetter for reviewer in
                                book.copyeditassignment_set.all()] or book.owner == request.user:
                return function(request, *args, **kwargs)
            else:
                messages.add_message(request, messages.ERROR,
    ...
    def wrap(request, *args, **kwargs):
        ...
        if 'press-editor' in user_roles:
            ...
            if request.user in book.onetaskers() or request.user in book.all_editors() or book.owner == request.user:
                ...
                raise exceptions.PermissionDenied
        elif not submission_id and (
            set(user_roles) & set(['copyeditor', 'typesetter', 'indexer'])):
            return function(request, *args, **kwargs)
        else:
            messages.add_message(request, messages.ERROR,
                                 'You need to have Press Editor, Book Editor or Series Editor level permission to view this page.')
