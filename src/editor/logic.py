...
def generate_digital_choices(digital_formats):
    return [('', '------')] + [
        (frmt.id, "%s - %s" % (frmt.name, frmt.get_file_type_display())) for
        frmt in digital_formats]


def generate_physical_choices(physical_formats):
    return [('', '------')] + [
        (frmt.id, "%s - %s" % (frmt.name, frmt.get_file_type_display())) for
        frmt in physical_formats]
...
def send_author_invite(submission, copyedit, email_text, sender,
    ...
    email.send_email(subject, context, from_email.value, submission.owner.email,
                     email_text, book=submission, attachment=attachment,
                     kind='copyedit')
...
def send_new_user_ack(submission, email_text, new_user, code):
    ...
    email.send_email(get_setting('new_user_subject', 'email_subject',
                     ...
                     book=submission, kind='general')


def handle_copyeditor_assignment(request, book, copyedit, files, due_date, note,
                                 email_text, requestor, attachment=None):
    try:
        ...
        log.add_log_entry(book=book, user=requestor, kind='copyedit',
                          message='Copyeditor %s %s assigned. Due %s' % (
                          copyedit.first_name, copyedit.last_name, due_date),
                          short_name='Copyedit Assignment')
        send_copyedit_assignment(book, new_copyeditor, email_text, requestor,
                                 attachment=attachment)
    except:
        messages.add_message(request, messages.WARNING,
                             'Copyedit Assignment for user <%s> already exists. User might already exist in one of the selected committees' % copyedit.username)


def handle_indexer_assignment(request, book, index, files, due_date, note,
                              email_text, requestor, attachment):
    try:
        ...
        log.add_log_entry(book=book, user=requestor, kind='index',
                          message='Indexer %s %s assigned. Due %s' % (
                          index.first_name, index.last_name, due_date),
                          short_name='Indexing Assignment')
    except:
        messages.add_message(request, messages.WARNING,
                             'Indexing Assignment for user <%s> already exists. User might already exist in one of the selected committees' % index.username)


def handle_typeset_assignment(request, book, typesetter, files, due_date,
                              email_text, requestor, attachment):
    try:
        ...
        log.add_log_entry(book=book, user=requestor, kind='typeset',
                          message='Typesetter %s %s assigned. Due %s' % (
                          typesetter.first_name, typesetter.last_name,
                          due_date), short_name='Typeset Assignment')
    except:
        messages.add_message(request, messages.WARNING,
                             'Typeset Assignment for user <%s> already exists. User might already exist in one of the selected committees' % typesetter.username)
...
def get_submission_tasks(book, user):
    ...
    copyedit_tasks = models.CopyeditAssignment.objects.filter(book=book,
    ...
    typeset_tasks = models.TypesetAssignment.objects.filter(
        ...
        Q(author_completed__isnull=False) & Q(
            editor_second_review__isnull=True)), book=book, requestor=user)

    for copyedit in copyedit_tasks:
        task_list.append({'type': 'copyedit', 'book': copyedit.book,
                          'task': 'Copyedit Review', 'date': copyedit.completed,
                          'title': copyedit.book.title,
                          'url': 'http://%s/editor/submission/%s/editing/view/copyeditor/%s/' % (
                          base_url, copyedit.book.id, copyedit.id)})

    for typeset in typeset_tasks:
        task_list.append({'type': 'typeset', 'book': typeset.book,
                          'task': 'Typesetting Review',
                          'date': typeset.completed,
                          'title': typeset.book.title,
                          'url': 'http://%s/editor/submission/%s/production/view/typesetter/%s' % (
...
def cancel_review_round(book):
    latest_round = models.ReviewRound.objects.filter(book=book).aggregate(
        max=Max('round_number'))
    cancel_round = models.ReviewRound.objects.get(book=book,
                                                  round_number=latest_round.get(
                                                      'max'))
    cancel_round.delete()


def handle_review_assignment(request, book, reviewer, review_type, due_date,
                             review_round, user, email_text, review_form,
    ...
    if created:
        ...
        log.add_log_entry(book=book, user=user, kind='review',
        ...
        messages.add_message(request, messages.WARNING,
                             'Review Assignment for user <%s> already exists. User might already exist in one of the selected committees' % reviewer.username)
        return obj


def handle_editorial_review_assignment(request, book, editors, access_key,
                                       due_date, user, email_text,
                                       attachment=None):
    ...
    obj.save()
    message = "A new Editorial Review Assignment for %s has been assigned to you by %s ." % (
    book.title, request.user.username)

    for editor in editors:
        if not obj.editorial_board.filter(username=editor.username).exists():
            notification = models.Task(assignee=editor, creator=request.user,
                                       text=message,
                                       workflow='editorial-review',
                                       editorial_review=obj, book=book)
            notification.save()
            ...
            messages.add_message(request, messages.WARNING,
                                 'Editorial Review Assignment for user <%s> already exists. User might already exist in one of the selected committees' % editor.username)
...
def send_review_request(book, review_assignment, email_text, sender,
    ...
    email.send_email(
        ...
        html_template=email_text, book=book, attachment=attachment,
